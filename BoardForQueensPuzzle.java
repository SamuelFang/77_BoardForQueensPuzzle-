...
public class BoardForQueensPuzzle {
    ...
    public boolean lastIsNg() {
        for (int current = 0; current < lastRankFilled; current++){
            if(filesWithQueens[current] == filesWithQueens[lastRankFilled])
				return true;
            if(Math.abs(filesWithQueens[lastRankFilled] - filesWithQueens[current]) == current + (lastRankFilled - current)
                && Math.abs(lastRankFilled - current) == current + (lastRankFilled - current)){
				return true;
			}
        }
        return false;
    ...
    public boolean accept() {
        if ((lastRankFilled == filesWithQueens.length - 1) & (lastIsNg() == false)) return true;
        return false;
    }
